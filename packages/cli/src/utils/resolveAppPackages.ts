 ...
 */
function typeAssertion({ ast, indexPath }: { ast: any; indexPath: string }): TypeAssertion {
...
function getAssertionTokenByTraverse(ast: any) {
...
function isPlugin(properties: any[]) {
  const [match] = properties.filter(property => property.key.name === 'install');

  return !!match;
};

function getComponentPackageImports({
  result,
  properties,
  indexPath,
  importDeclarations,
...
function getIndexPath(entry: string) {
...
function parseEntry({ ast, package: module, indexPath }: ParseEntryOption) {
  if (!ast.program) {
  ...
  [, config] = config.match(reg) || [void 0, config];
  [, value] = value.match(reg) || [void 0, value];
  [, component] = component.match(reg) || [void 0, component];
  [, event] = event.match(reg) || [void 0, event];
...
function getASTTokenByTraverse({ ast, indexPath }: { ast: any; indexPath: string }) {
  let config = '';
  let value = '';
  let event = '';
  ...
  recast.types.visit(ast, {
    visitImportDeclaration(p) {
      ...
      importComponentToken.push(specifiers?.[0].local?.name as string || '');
    ...
    visitExportNamedDeclaration(p) {
      const { node } = p;
      const { specifiers, source } = node;
      const name = (specifiers?.[0].exported.name as string).toLowerCase();
