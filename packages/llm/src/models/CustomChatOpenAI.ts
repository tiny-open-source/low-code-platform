...
>
  ...
  invocationParams(options: any) {
    function isStructuredToolArray(tools: any[]) {
      return (
        tools !== undefined
  ...
  async _generate(
    ...
    if (params.stream) {
      ...
      const generations = Object.entries(finalChunks)
        .sort(([aKey], [bKey]) => Number.parseInt(aKey, 10) - Number.parseInt(bKey, 10))
        .map(([_, value]) => value);
   ...
   */
  async getEstimatedTokenCountFromPrompt(messages: any[], functions: any, function_call: any) {
   ...
   */
  async getNumTokensFromGenerations(generations: any[]) {
  ...
  async getNumTokensFromMessages(messages: any[]) {
    let totalCount = 0;
  ...
  withStructuredOutput(outputSchema: any, config: any) {
    let schema;
    let name;
    let method;
    let includeRaw;
    ...
    const parserAssign = RunnablePassthrough.assign({
      parsed: (input, config) => outputParser.invoke(input.raw as any, config),
    });
